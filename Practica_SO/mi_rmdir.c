...
/**
Función: borra un directorio, llamando a la función mi_unlink()
Parámetros: 
...
int main(int argc, char *argv[]) {
    ...
    char *ruta = argv[2];
    // Comprobamos si la ruta es un directorio (no puede ser el directorio raíz)
    if (ruta[strlen(ruta)-1] != '/') {
        fprintf(stderr, RED "Error: La ruta proporcionada no referencia un directorio\n" RESET);
        return FALLO;
    ...
    // Llamamos a la función mi_unlink()
